...
if (!class_exists("TomM8")) {
  class TomM8 {
    ...
    function get_query_string_value($name, $index = -1) {
      if ($index == -1) {
        if (isset($_POST[$name])) {
          return fix_http_quotes($_POST[$name]);
        } else if (isset($_GET[$name])) {
          return fix_http_quotes($_GET[$name]);
            ...
            $_POST[$name] = fix_http_quotes($data);
            return fix_http_quotes($data);
        } else {
          return "";
        }
      } else {
        ...
        if (isset($_POST[$name][$index])) {
          return fix_http_quotes($_POST[$name][$index]);
        } else if (isset($_GET[$name][$index])) {
          return fix_http_quotes($_GET[$name][$index]);
            ...
            $_POST[$name][$index] = fix_http_quotes($data);
            return fix_http_quotes($data);
        } else {
          return "";
        }
      ...
      if (is_array($uploadfiles)) {
        ...
        foreach ($uploadfiles['name'] as $key => $value) {
          ...
          if ($uploadfiles['error'][$key] == 0) {
            ...
            update_record_by_id("posts", array("guid" => get_option("siteurl").$matches[0][0]), "ID", $attach_id);
            // echo $filedest;
          }
        }   
      }
    ...
    function generate_datatable($table_name, $fields_array, $primary_key_name, $where_clause, $order_array = array(), $limit_clause = "", $page_name, $display_show = true, $display_edit = true, $display_delete = true, $sortable_columns = false, $paginate_table = false, $date_format = "Y-m-d", $filter_arrays = array()) {
      ...
      if (!is_array($fields_array)) {
        ...
        if (count($filter_arrays) > 0) {
          if (get_query_string_value($table_name."_filters") != "") {
            $filters = explode(",", get_query_string_value($table_name."_filters"));
            foreach ($filters as $filter) {
              if (get_query_string_value("filter_".$filter) != "") {
                if (!(isset($_POST["action"]) && $_POST["action"] == "Reset")) {
                  array_push($extra_where, $filter." LIKE '%".get_query_string_value("filter_".$filter)."%'");  
                }
              }
            }
        ...
        $results = get_results($table_name, $fields_array, $where_clause, $order_array, $limit_clause.$offset_clause);
        
        $total_count = count(get_results($table_name, $fields_array, $where_clause));
              ...
              <fieldset>
                ...
                foreach($filter_arrays as $filter_array) { 
                  foreach($filter_array as $key => $value) { 
                    if (get_query_string_value("filter_".$key) != "") {
                      if (isset($_POST["action"]) && $_POST["action"] == "Reset") {
                        $_POST["filter_".$key] = "";
                      }
                      $params_filter .= "&filter_".$key."=".get_query_string_value("filter_".$key);
                    }
        ...
        if (count($results) > 0) { 
          if ($paginate_table) { generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, "top"); } ?>
            <table class="data">
              <thead>
                <tr>
                  <?php foreach($fields_array as $field_name) { ?>
                    <th class='<?php echo(strtolower(str_replace("_", "-", $field_name))); ?>'>
                      ...
                      <?php } ?>
                      <?php echo(titlize_str($field_name)); ?>
                      <?php if ($sortable_columns) {
                  ...
                  <tr>
                    <?php foreach($fields_array as $field_name) { ?>
                      <td class='<?php echo(esc_html(strtolower(str_replace("_", "-", $field_name)))); ?>'>
                        <?php 
                          if (is_valid_datetime($result->$field_name)) {
            ...
            <?php if ($paginate_table) { generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, "bottom"); } ?>
        <?php } else {
    ...
    function generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, $pagination_class) {
      ...
      $total_number_pages = intval($total_count / $limit_clause);
        $params_filter = "";
        $filters = explode(",", get_query_string_value($table_name."_filters"));
        $params_filter .= "&".$table_name."_filters=".get_query_string_value($table_name."_filters");
        foreach ($filters as $filter) {
          if (get_query_string_value("filter_".$filter) != "") {
            $params_filter .= "&filter_".$filter."=".get_query_string_value("filter_".$filter);
          }
        }
    ...
    function generate_displayview($table_name, $fields_array, $id_column_name, $id) {
      if (!is_array($fields_array)) {
        ...
        $result = get_row_by_id($table_name, $fields_array, $id_column_name, $id);
        echo("<dl>");
    ...
    function validate_form($validations_array) {
      ...
      foreach ($validations_array as $key => $value) {
        if (is_array(get_query_string_value($key))) {
          $index = 0;
          foreach (get_query_string_value($key) as $sub_value) {
            if (validate_value($value, $sub_value, $key."_".$index."_error") == false) {
              $validate_form = false;
            }
            $index++;
          }
          ...
          if (preg_match("/required/i", $value) && isset($_POST[$key."_0"])) {
            ...
            if (is_array(get_query_string_value($key."_0"))) {
              // For checkbox fields.
              $index = 0;
              foreach ($_POST["validation_0"] as $row) {
                ...
                if (validate_value($value, $data, $key."_".$index."_error") == false) {
                  echo $index;
                  $validate_form = false;
                }
                $index++;
              ...
              if (validate_value($value, $data, $key."_error") == false) {
                $validate_form = false;
              }
            }
          } else {
            if (validate_value($value, get_query_string_value($key), $key."_error") == false) {
              $validate_form = false;
            }
          }
        }
    ...
    function add_form_field($instance, $field_type, $field_label, $field_id, $field_name, $field_attributes = array(), $container_element, $container_attributes = array(), $value_options = array(), $field_index = -1) {
      ...
      if ($instance == null && preg_match("/^tomm8te_admin_option::/", $field_name)) {
        ...
        if (count($_POST) > 0) {
          if ($field_index >= 0) {
            $field_value = get_query_string_value($field_name, $field_index);
          } else {
            $field_value = get_query_string_value($field_name);
          }
        }
      } else {
      ...
      if ($field_type == "text") {
        ...
        if (count($value_options) == 1) {
          ...
          foreach($value_options as $key => $option) {
            ...
            $field_value = get_query_string_value($field_name."_".$i, $field_index);
            $field_value = str_replace("&amp;", "&", htmlentities(htmlentities($field_value, ENT_NOQUOTES), ENT_QUOTES));
            $checked_value = "";
            if (count($_POST) == 0) {
    ...
    function add_option_form_field($field_type, $field_label, $field_id, $option_name, $field_attributes = array(), $container_element, $container_attributes = array(), $value_options = array(), $field_index = -1) {
      ...
      add_form_field(null, $field_type, $field_label, $field_id, "tomm8te_admin_option::".$option_name, $field_attributes, $container_element, $container_attributes, $value_options, $field_index);
    }
